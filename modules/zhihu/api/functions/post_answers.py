...
from db.mysql.mysql_jdbc import insert, select_list, create_pool, close_pool, select_count
        ...
        try:
            ...
            async with async_playwright() as p:
                ...
                for i in locators_all:
                    handle = await i.element_handle()  # ERROR - An error occurred: Locator.element_handle: Timeout 30000ms exceeded.：waiting for locator("div[role=\"list\"]").locator("div[class=\"css-vurnku\"]").locator("a").nth(38)
                ...
                for index, href in enumerate(hrefs):
                    ...
                    if check_selector is not None:
                        ...
                        continue
                    # 检测一下是否已经回答过
                    check_selector = await  self.page.query_selector('a[type="button"]:has-text("编辑回答")')
                    if check_selector is not None:
                        logging.info(f"已经回答过了，跳过")
                        continue
                    # 问题标题
                    ...
                    # 检测重复,是否已经回答过这个问题
                    count = await select_count(pool=self.pool, table_name='post_answers', condition=[
                        {'field': 'account_name', 'value': self.account_name, 'op': 'eq'},
                        {'field': 'question_title', 'value': gpt_param["question_title"], 'op': 'ct'},
                    ])
                    if count > 0:
                    ...
                    try:
                        if pictures:
                            ...
                            for i in range(3):
                                try:
                                    element_handle = await self.page.query_selector('div:has-text("上传图片")')
                                    if element_handle:
                                        break
                                    ...
                                    logging.info(f"无法点击上传图片，再次尝试:{i}")
                            ...
                            try:
                                ...
                                page_path = await screenshot_page(page=self.page,
                                                                  file_name=datetime.datetime.now().strftime(
                                                                      '%Y-%m-%d_%H-%M-%S-%f'))
                                # 发送邮件
                                await email_notification(message=f"{exc_info}",
                                                         image_paths=[page_path])
...
"""
    ...
    async def run_task(pool, html_lock, resource_lock, account, accounts):
        ...
        if len(find_list) > 0:
            find = find_list[0]
            recommend = PostAnswers(pool=pool, html_lock=html_lock, command=find['command'],
                                    account_username=account['username'],
                                    account_name=account['account_name'],
                                    user_data_dir_path=account['user_data_dir_path'],
                                    remote_debugging_port=account['remote_debugging_port'],
                                    browser_type=account['browser_type'], label="擅长话题", draft=False, hide=False)
            # 初始化资源管理器
