...
token = "d18b98fb5e795125950300805cdfbe59"
...
class JiMengAPITwo:
    ...
    async def main_images(description, timeout=60):
        ...
        try:
            result = await JiMengAPITwo.chat_images(pool=pool, description=description, timeout=timeout)
            return result
        finally:
    ...
    async def main_chat(question, prompt="", timeout=60):
        pool = await create_pool(db='gpt_4_free')
        try:
            result = await JiMengAPITwo.chat(pool=pool, question=question, prompt=prompt, timeout=timeout)
            return result
        finally:
            await close_pool(pool)
...
if __name__ == '__main__':
    asyncio.run(JiMengAPITwo.main_chat("小樱"))
    asyncio.run(JiMengAPITwo.main_images("小樱"))
