...
token = "ES0adnpfbGzzg_8yn1gsBH1opRO0qadzXjCc$oygNYkOTW6UdKeS2rTszkpRv8wya90tHVj31BBa0"
...
class QwenAPIThree:
    ...
    async def main_images(description, timeout=60):
        ...
        try:
            result = await QwenAPIThree.chat_images(pool=pool, description=description, timeout=timeout)
            return result
        finally:
    ...
    async def main_chat(question, prompt="", timeout=60):
        pool = await create_pool(db='gpt_4_free')
        try:
            result = await QwenAPIThree.chat(pool=pool, question=question, prompt=prompt, timeout=timeout)
            return result
        finally:
            await close_pool(pool)
