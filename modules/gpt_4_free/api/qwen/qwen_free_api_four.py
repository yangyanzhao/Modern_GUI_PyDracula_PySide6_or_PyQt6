...
class QwenAPIFour:
    ...
    async def main_images(description, timeout=60):
        ...
        try:
            result = await QwenAPIFour.chat_images(pool=pool, description=description, timeout=timeout)
            return result
        finally:
    ...
    async def main_chat(question, prompt="", timeout=60):
        pool = await create_pool(db='gpt_4_free')
        try:
            result = await QwenAPIFour.chat(pool=pool, question=question, prompt=prompt, timeout=timeout)
            return result
        finally:
            await close_pool(pool)
