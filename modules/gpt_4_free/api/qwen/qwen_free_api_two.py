...
token = "ZBhZ4C6w9300AfUnfgbGbzpf8Gz1g_BynogsOH1apRX0qcdzyjCR$oafxBTl*Ldz4jhjekV4beVI0"
...
class QwenAPITwo:
    ...
    async def main_images(description, timeout=60):
        ...
        try:
            result = await QwenAPITwo.chat_images(pool=pool, description=description, timeout=timeout)
            return result
        finally:
    ...
    async def main_chat(question, prompt="", timeout=60):
        pool = await create_pool(db='gpt_4_free')
        try:
            result = await QwenAPITwo.chat(pool=pool, question=question, prompt=prompt, timeout=timeout)
            return result
        finally:
            await close_pool(pool)
