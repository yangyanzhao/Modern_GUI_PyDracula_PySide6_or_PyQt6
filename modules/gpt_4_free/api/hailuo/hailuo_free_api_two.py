...
token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..QwixgYEy4qO7eJyboOQny-Y66-ryeg8SLqsMq1aVHjg"
...
class HailuoAPITwo:
    ...
    async def main_chat_audio_speech(content, voice):
        ...
        try:
            result = await HailuoAPITwo.chat_audio_speech(content=content, voice=voice)
            return result
        finally:
    ...
    async def main_chat(question, prompt="", timeout=60):
        pool = await create_pool(db='gpt_4_free')
        try:
            result = await HailuoAPITwo.chat(pool=pool, question=question, prompt=prompt, timeout=timeout)
            return result
        finally:
            await close_pool(pool)
...
if __name__ == '__main__':
    asyncio.run(HailuoAPITwo.main_chat(question="你是谁？", prompt=""))
