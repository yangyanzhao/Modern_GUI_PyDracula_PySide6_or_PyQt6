...
token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..-U1zfh7BX6MWWDi4eWYEGOQmOoqeJk9lVcftAcNc52k"
...
class HailuoAPIOne:
    ...
    async def main_chat_audio_speech(content, voice):
        ...
        try:
            result = await HailuoAPIOne.chat_audio_speech(content=content, voice=voice)
            return result
        finally:
    ...
    async def main_chat(question, prompt="", timeout=60):
        pool = await create_pool(db='gpt_4_free')
        try:
            result = await HailuoAPIOne.chat(pool=pool, question=question, prompt=prompt, timeout=timeout)
            return result
        finally:
            await close_pool(pool)
...
if __name__ == '__main__':
    asyncio.run(HailuoAPIOne.main_chat_audio_speech(content="砥砺前行，奋勇向前，迎难而上，坚如磐石，越挫越勇，勇猛精进", voice="Linzhiling_hailuo"))
