...
token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJkZDkwMjA4YzcxYWE0MTM2OGY4NTllYWZhMTc1OTgzZCIsImV4cCI6MTczNjIzMDY3NywibmJmIjoxNzIwNjc4Njc3LCJpYXQiOjE3MjA2Nzg2NzcsImp0aSI6IjMxMzY2M2ZhYTQ5NDRkOTE4MDdlMTdjYTFhMGNjZDYyIiwidWlkIjoiNjY4Zjc5MTQzM2EzNGFjODljYTdkNDJmIiwidHlwZSI6InJlZnJlc2gifQ._LlUtf2w8_Hn9hpbsuRDTntKB8SZw8VYYqqClEZ6HfI"
...
class GlmAPITwo:
    ...
    async def main_images(description, timeout=60):
        ...
        try:
            result = await GlmAPITwo.chat_images(pool=pool, description=description, timeout=timeout)
            return result
        finally:
    ...
    async def main_chat(question, prompt="", timeout=60):
        pool = await create_pool(db='gpt_4_free')
        try:
            result = await GlmAPITwo.chat(pool=pool, question=question, prompt=prompt, timeout=timeout)
            return result
        finally:
            await close_pool(pool)
...
if __name__ == '__main__':
    asyncio.run(GlmAPITwo.main_chat("现在几点了"))
    asyncio.run(GlmAPITwo.main_images("现在几点了"))
