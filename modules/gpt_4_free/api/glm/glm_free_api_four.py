...
token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiI4NTY5N2Q4NmFjYjk0MjRhYWIwMTUyYmVmYmZkYzBmNSIsImV4cCI6MTczODkwMjEwMiwibmJmIjoxNzIzMzUwMTAyLCJpYXQiOjE3MjMzNTAxMDIsImp0aSI6ImZhNzI2NzBlOGJmNzRjMTBhZDk4MDExODU2MmI3OTAwIiwidWlkIjoiNjZiODNjNTVkZWFmOWFiMmM0NTc2ZjQ3IiwidHlwZSI6InJlZnJlc2gifQ.NlwGaELtrHCy2Q0n4P8yodCqAYhyeicP_U-Hd-KAoRA"
...
class GlmAPIFour:
    ...
    async def main_images(description, timeout=60):
        ...
        try:
            result = await GlmAPIFour.chat_images(pool=pool, description=description, timeout=timeout)
            return result
        finally:
    ...
    async def main_chat(question, prompt="", timeout=60):
        pool = await create_pool(db='gpt_4_free')
        try:
            result = await GlmAPIFour.chat(pool=pool, question=question, prompt=prompt, timeout=timeout)
            return result
        finally:
            await close_pool(pool)
...
if __name__ == '__main__':
    asyncio.run(GlmAPIFour.main_chat("赵飞燕"))
    asyncio.run(GlmAPIFour.main_images("赵飞燕"))
