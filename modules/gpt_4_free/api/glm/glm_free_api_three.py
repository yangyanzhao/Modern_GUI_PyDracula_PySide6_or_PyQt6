...
token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiI0OWI3NjlkMzkxODQ0YzBmYmM0ZTc1YWNmMDk5MzRlMCIsImV4cCI6MTczNjQwODE2OCwibmJmIjoxNzIwODU2MTY4LCJpYXQiOjE3MjA4NTYxNjgsImp0aSI6IjE3NjllNzc5NmY4NzQ3MGQ4MjUxNGVlNTY5Y2Y3Mjc4IiwidWlkIjoiNjY5MjJlNjdkY2ZlM2FhZWQxYTJkYTM2IiwidHlwZSI6InJlZnJlc2gifQ.Dc3frMeaFe5TKLhoC_WPT2OZES53AVeijLUepdHaKZA"
...
class GlmAPIThree:
    ...
    async def main_images(description, timeout=60):
        ...
        try:
            result = await GlmAPIThree.chat_images(pool=pool, description=description, timeout=timeout)
            return result
        finally:
    ...
    async def main_chat(question, prompt="", timeout=60):
        pool = await create_pool(db='gpt_4_free')
        try:
            result = await GlmAPIThree.chat(pool=pool, question=question, prompt=prompt, timeout=timeout)
            return result
        finally:
            await close_pool(pool)
...
if __name__ == '__main__':
    # asyncio.run(GlmAPIThree.main_chat("刘亦菲"))
    run = asyncio.run(GlmAPIThree.main_images("生成疯狂的麦克斯之狂暴女神的性感照片"))
    print(run)
