...
class SparkAPIOne:
            ...
            try:
                async with session.post(URL, headers=headers, data=payload, timeout=timeout) as response:
                    if response.status == 200:
                        ...
                        choices = result['choices']  # 这里报过错：KeyError: 'choices'
                        content = choices[0]['message']['content']
                        # 去掉搜索的内容
                        sub = await SparkAPIOne.remove_identifiers_and_content(content, r'```searchSource', '```')
                        handle_answer = sub
                        # 1正常回答2超时3异常
                        status = 1
                        return sub
    ...
    def chat_images(description):
        ...
        try:
            response = requests.request("POST", URL_AI_IMAGES, headers=headers, data=payload)
            result: dict = json.loads(response.text)
            choices = result['data']
    ...
    async def main_images(description):
        ...
        try:
            result = await SparkAPIOne.chat_images(description=description)
            return result
        finally:
    ...
    async def main_chat(question, prompt="", timeout=60):
        pool = await create_pool(db='gpt_4_free')
        try:
            result = await SparkAPIOne.chat(pool=pool, question=question, prompt=prompt, timeout=timeout)
            return result
        finally:
            await close_pool(pool)
