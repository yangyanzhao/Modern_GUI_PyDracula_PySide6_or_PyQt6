...
token = "ad8e8517-d07a-46f9-88bd-90848756db5a"
...
class SparkAPIThree:
            ...
            try:
                async with session.post(URL, headers=headers, data=payload, timeout=timeout) as response:
                    if response.status == 200:
                        ...
                        # 去掉搜索的内容
                        sub = await SparkAPIThree.remove_identifiers_and_content(content, r'```searchSource', '```')
                        handle_answer = sub
                        # 1正常回答2超时3异常
                        status = 1
                        return sub
    ...
    async def main_images(description):
        ...
        try:
            result = await SparkAPIThree.chat_images(description=description)
            return result
        finally:
    ...
    async def main_chat(question, prompt="", timeout=60):
        pool = await create_pool(db='gpt_4_free')
        try:
            result = await SparkAPIThree.chat(pool=pool, question=question, prompt=prompt, timeout=timeout)
            return result
        finally:
            await close_pool(pool)
