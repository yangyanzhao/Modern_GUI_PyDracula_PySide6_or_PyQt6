...
from widgets.dayu_widgets.mixin import property_mixin
from widgets.dayu_widgets.popup import MPopup
from widgets import dayu_widgets as utils
...
class ScrollableMenuBase(QtWidgets.QMenu):
    ...
    def setMaxItemCount(self, count):
        ...
        self.defaultItemHeight = style.sizeFromContents(QStyle.ContentsType.CT_MenuItem, opt, size, self).height()

        if not count:
            self.setMaximumHeight(self._maximumHeight)
        else:
            fw = style.pixelMetric(QStyle.PixelMetric.PM_MenuPanelWidth, None, self)
    ...
    def resizeEvent(self, event):
        ...
        style = self.style()
        fw = style.pixelMetric(style.PixelMetric.PM_MenuPanelWidth, None, self)  # 获取菜单的面板宽度
        hmargin = style.pixelMetric(style.PixelMetric.PM_MenuHMargin, None, self)  # 获取水平边距
        vmargin = style.pixelMetric(style.PixelMetric.PM_MenuVMargin, None, self)  # 获取垂直边距

        leftMargin = fw + hmargin  # 计算左侧边距
        topMargin = fw + vmargin  # 计算顶部边距
        bottomMargin = fw + vmargin  # 计算底部边距
        contentWidth = self.width() - (fw + hmargin) * 2  # 计算内容宽度
    ...
    def paintEvent(self, event):
        ...
        menuOpt.state = QStyle.StateFlag.State_None
        ...
        for action, actionRect in self.iterActionRects():
            ...
            style.drawControl(QStyle.ControlElement.CE_MenuItem, opt, qp, self)
        qp.restore()
        ...
        for isDown, enabled, scrollRect in upData, downData:
            ...
            scrollOpt.state |= style.StateFlag.State_DownArrow if isDown else style.StateFlag.State_UpArrow
            scrollOpt.checkType = QtWidgets.QStyleOptionMenuItem.NotCheckable
            scrollOpt.maxIconWidth = scrollOpt.tabWidth = 0
            scrollOpt.rect = scrollRect
            scrollOpt.menuItemType = QtWidgets.QStyleOptionMenuItem.Scroller
            if enabled:
        ...
        menuOpt.menuItemType = QtWidgets.QStyleOptionMenuItem.EmptyArea
        menuOpt.checkType = QtWidgets.QStyleOptionMenuItem.NotCheckable
